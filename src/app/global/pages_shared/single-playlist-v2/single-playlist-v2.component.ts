...
export class SinglePlaylistV2Component implements OnInit, OnDestroy {
    ...
    private getAssets() {
        ...
        forkJoin(requests).subscribe({
            next: ([dealerContents, floatingContents]: [{ contents?: API_CONTENT_V2[]; page?: any; message?: string }, { iContents: API_CONTENT_V2[]; page: any }]) => {
                ...
                if (floatingContents && floatingContents.iContents.length) this.floatingAssets = floatingContents.iContents;
                const totalAssetsLength = this.assets.length + this.floatingAssets.length;

                this.playlistControls = this.playlistControls.map((p) => {
                    /** Add Bulk Button Actions Here */
                    if (p.action == pActions.addContent) return { ...p, icon: totalAssetsLength ? 'fas fa-plus' : 'fas fa-ban', disabled: !totalAssetsLength };
                    return p;
                });
    ...
    public playlistContentSelected(playlistContentId: string) {
        if (this.selectedPlaylistContents.includes(playlistContentId)) this.selectedPlaylistContents = this.selectedPlaylistContents.filter((p) => p !== playlistContentId);
    ...
    private setBulkControlsState() {
        this.playlistControls = this.playlistControls.map((p) => {
            /** Add Bulk Button Actions Here */
            if (p.action == pActions.bulkModify || p.action == pActions.bulkDelete) return { ...p, disabled: this.selectedPlaylistContents.length < 1 };
    ...
    private showConfirmationDialog(confirmationType: 'success' | 'warning' | 'error', actionType: string, title: string, message: string, returnMsg: string) {
    ...
    private showRemoveContentDialog(content?: API_CONTENT_V2) {
        ...
        this._dialog
            ...
            .subscribe((response: string | boolean) => {
                ...
                const childContentsToDelete = this.playlistContents.filter((c) => c.parentId && idsToDelete.includes(c.parentId)).map((c) => c.playlistContentId);
        ...
        this._playlist.swapPlaylistContent(playlistContentToSwap, isFrequencySwap).subscribe({
            next: (res: { content: API_CONTENT_V2; plContent: API_UPDATED_PLAYLIST_CONTENT }) => {
                this.playlistContents = this.playlistContents
                    ...
                    .sort((a, b) => this.playlistSortableOrder.indexOf(a.playlistContentId) - this.playlistSortableOrder.indexOf(b.playlistContentId));
