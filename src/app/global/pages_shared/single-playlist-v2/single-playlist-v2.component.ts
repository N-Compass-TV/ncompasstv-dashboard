...
export class SinglePlaylistV2Component implements OnInit, OnDestroy {
            ...
            next: ([dealerContents, floatingContents]: [
                ...
                this.playlistControls = this.playlistControls.map((p) => {
                    ...
                    if (p.action == pActions.addContent)
                        return {
                            ...p,
                            icon: totalAssetsLength ? 'fas fa-plus' : 'fas fa-ban',
                            disabled: !totalAssetsLength,
                        };
                    return p;
                });
    ...
    private getAssetCount(): void {
        ...
        this.videoCount = this.playlistContents.filter((i) =>
            fileTypes('video').includes(i.fileType.toLowerCase()),
        ).length;
        this.imageCount = this.playlistContents.filter((i) =>
            fileTypes('image').includes(i.fileType.toLowerCase()),
        ).length;
        this.feedCount = this.playlistContents.filter((i) =>
    ...
    private savePlaylistContentUpdates(data: SavePlaylistContentUpdate, savingPlaylist = true) {
        ...
        if (data.blacklistUpdates && data.blacklistUpdates.length)
