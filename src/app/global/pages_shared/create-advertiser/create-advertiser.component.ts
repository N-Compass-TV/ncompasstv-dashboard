...
import { AdvertiserService, AuthService, CategoryService, DealerService, FastEdgeService, HelperService, LocationService, MapService } from 'src/app/global/services';

import { API_DEALER, API_CREATE_ADVERTISER, API_GOOGLE_MAP, API_PARENT_CATEGORY, City, PAGING, UI_ROLE_DEFINITION, UI_TABLE_DEALERS } from 'src/app/global/models';
    ...
    newAdvertiserProfile() {
        ...
        if ((this.is_creating_advertiser = true)) {
            this._advertiser
                ...
                .subscribe(
                    (data: any) => {
                        this.openConfirmationModal('success', 'Advertiser Profile Created!', 'Hurray! You successfully created an Advertiser Profile', data.id);
                    },
                    (error) => {
                        this.is_creating_advertiser = false;
                        this.openConfirmationModal('error', 'Advertiser Profile Creation Failed', "Sorry, There's an error with your submission", null);
                    },
                );
        }
    }
