    ...
    private async getAvailableLicenses() {
        ...
        try {
            const firstPageRequest = await this._license
                ...
                .map((response) => {
                    response.paging.entities = response.paging.entities.filter((license: API_LICENSE_PROPS) => !license.hostId || (license.hostId && license.hostId.length <= 0));
                    return response;
                })
                .toPromise();
        ...
        merged = merged.concat((firstPageResult.paging.entities as API_LICENSE_PROPS[]).filter((license) => !license.hostId || (license.hostId && license.hostId.length <= 0)));
