...
export class ChangeTemplateComponent implements OnInit, OnDestroy {
    ...
    private subscribeToChangeTemplate() {
        this.changeTemplateForm.controls.template.valueChanges.pipe(takeUntil(this._unsubscribe)).subscribe((response: { name: string; id: string }) => {
    ...
    protected get _formControls(): FORM_CONTROL[] {
        return [{ name: 'template', label: 'Select Template', type: 'select', options: this.mappedTemplates, option_label: 'name', required: true }];
    }
}
