...
export class AuthService {
    ...
    authenticate_user(data) {
        return this._http.post<USER_LOGIN>(`${environment.base_uri}${environment.auth.api_login}`, data, this.http_options).pipe(
            map((current_user) => {
    ...
    //Refresh Token
    refresh_token() {
        return this._http.post<JWT_TOKEN>(`${environment.base_uri}${environment.auth.api_refresh}`, JSON.stringify(this.current_user_value.jwt), this.http_options).pipe(
            map((data) => {
