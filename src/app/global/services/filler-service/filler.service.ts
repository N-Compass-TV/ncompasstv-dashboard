...
export class FillerService extends BaseService {
    ...
    get_filler_group_dealer_admin_view(id: string, page: number, key: string, pageSize = 11, column = '', order = '', userview?) {
        let url = '';
        if (userview == 'dealeradmin') url = `${this.getters.api_get_dealeradmin_filler_groups_view}?page=${page}&pageSize=${pageSize}&sortColumn=${column}&sortOrder=${order}`;
        else url = `${this.getters.api_get_dealer_filler_groups_admin_view}?id=${id}&page=${page}&pageSize=${pageSize}&sortColumn=${column}&sortOrder=${order}`;
