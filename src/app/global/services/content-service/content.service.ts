...
export class ContentService extends BaseService {
    ...
    get_unused_contents(id, type = '', key?) {
        return this.getRequest(`${this.getters.api_get_usused_contents}` + '?dealerid=' + `${id}` + '&filetype=' + `${type}` + '&search=' + `${key}` + '&pageSize=0');
    }
    ...
    generate_content_logs_report(data: { contentId: string; start: string; end: string }) {
        return this.getRequest(`${this.getters.api_get_logs_based_reports}` + '?contentId=' + `${data.contentId}` + '&startDate=' + `${data.start}` + '&endDate=' + `${data.end}`);
    }
