...
export class TagService extends BaseService {
    ...
    searchOwnersByTagType({ keyword = null, tagId = null, typeId = null, page = 1, role = 1 }, isDealer): Observable<{ tags?: TAG_OWNER[]; paging?: PAGING; message?: string }> {
        let url_split = this.getters.search_owner_tags.split('/');
        let new_url = url_split[0] + '/dealer/' + url_split[1];
        let final_url = !isDealer ? this.getters.search_owner_tags : new_url;
    ...
    searchDealerData({ keyword = null, tagId = null, typeId = null, page = 1, role = 1 }, isDealer): Observable<{ tags?: TAG_OWNER[]; paging?: PAGING; message?: string }> {
    ...
    updateTag(tagId: string, name: string, tagColor: string, updatedBy: string, description?: string, exclude = 0): Observable<{ message: string; tag?: TAG[] }> {
        const body: TAG = {
            tagId,
            name,
            tagColor,
